...
!!$  character(len=180),parameter,private :: dir_dens_path = "/home/kjahn/cubep3m_reion/" 
!!$  character(len=180),parameter,private :: dir_dens_path = "/work/01113/tg803913/" 
!!$  character(len=180),parameter,private :: dir_dens_path = "/work/00506/ilievit/114Mpc_WMAP3+/" 
  character(len=180),parameter,private :: dir_dens_path = "../morphed_densities/" 
  !> Name of directory with density files
  !character(len=180),parameter,private :: dir_dens_name= "coarser_densities/"
!!$  character(len=180),parameter,private :: dir_dens_name= "SPH_1024_37Mpc_halo_removed_Coarse/"
!!$  character(len=180),parameter,private :: dir_dens_name= "coarser_data_SPH_1024_37Mpc/halos_removed/"
!!$  character(len=180),parameter,private :: dir_dens_name= "c2ray_114Mpc_f10_150S_256_varC/coarser_densities/halos_removed/"
  character(len=180),parameter,private :: dir_dens_name= "halos_removed/"

  !> Path to directory containing directory with density(halos included) files:
!!$  character(len=180),parameter,private :: dir_densWH_path = "/home/kjahn/cubep3m_reion/" 
!!$  character(len=180),parameter,private :: dir_densWH_path = "/work/01113/tg803913/" 
!!$  character(len=180),parameter,private :: dir_densWH_path = "/work/00506/ilievit/114Mpc_WMAP3+/" 
  character(len=180),parameter,private :: dir_densWH_path = "../morphed_densities/"
  !> Name of directory with density (halos included) files
!!$  character(len=180),parameter,private :: dir_densWH_name= "SPH_1024_37Mpc_halo_included_Coarse/"
!!$  character(len=180),parameter,private :: dir_densWH_name= "coarser_data_SPH_1024_37Mpc/halo_included/"
!!$  character(len=180),parameter,private :: dir_densWH_name= "c2ray_114Mpc_f10_150S_256_varC/coarser_densities/halos_included/"
  character(len=180),parameter,private :: dir_densWH_name= "halos_included/"

  !> Path to directory containing directory with source files:
!!$  character(len=180),parameter,private :: dir_src_path = "/home/kjahn/cubep3m_reion/" 
!!$  character(len=180),parameter,private :: dir_src_path = "/work/01113/tg803913/"
!!$  character(len=180),parameter,private :: dir_src_path = "/home/kjahn/cubep3m_reion/6144_114Mpc/64grid/"
!!$  character(len=180),parameter,private :: dir_src_path = "../../cubep3m_6144_114Mpc/256grid/"
  character(len=180),parameter,private :: dir_src_path = "../"
  !> Name of directory with source files
!!$  character(len=180),parameter,private :: dir_src_name= "RT_postprocessing_37Mpc/"
!!$  character(len=180),parameter,private :: dir_src_name= "RT_postprocessing_37Mpc/"
  character(len=180),parameter,private :: dir_src_name= "sources/"

  !> Format of files (unformatted or binary)
#ifdef IFORT
  ! ifort standard for "binary"
  character(len=*),parameter :: densityformat="binary"
  character(len=*),parameter :: densityWHformat="binary"
  ...
  character(len=*),parameter :: densityWHformat="unformatted"
...
!!$  !> Format of density (halos included) file (unformatted or binary)
!!$  character(len=15),parameter :: densityWHformat="binary"
...
!!$  character(len=15),parameter :: clumpingformat="binary"
  ...
  !> density (halos included) file with header?
  logical,parameter :: densityWHheader=.true.
...
!  real(kind=dp),parameter,public :: tscale= 2.d0/(3.d0*sqrt(Omega0)*H0)
  real(kind=dp),parameter,public :: tscale= 2.d0/(3.d0*Omega0**0.5d0*H0)

  ! from Kyungjin: CARE SHOULD BE TAKEN WHEN MINIHALOS ARE IMPLEMENTED.
  !                NO-MINIHALO REDSHIFTS ARE LESS FREQUENT, WHILE MINIHLO
  !                REDSHIFTS ARE MORE FREQUENT. FINELY SPACED (MH) REDSHIFT
  !                LIST WILL THEN BE USED AS THE MAIN REDSHIFT LIST, WHILE
  !                ORIGINAL N-BODY(NO-MH) REDSHIFT LIST WILL BE USED AS THE 
  !                SECONDARY LIST.
  !                INTERMEDIATE MASS HALOS (10^8 < M/MSUN < 10^9) WILL BE
  !                SUPPRESSED ONLY AT SECONDARY, COARSE (N-BODY) REDSHIFTS.
...
#ifdef MH
  integer, public :: NumZred_coarse               !< number of redshifts
  real(kind=dp),dimension(:),allocatable,public :: zred_array_coarse !< array of redshifts 
#endif
  ...
  character(len=9),public :: id_str       !< resolution dependent string
  
  character(len=180),public :: dir_dens !< Path to directory with density files
  character(len=180),public :: dir_densWH !< Path to directory with density (halos included) files
...
contains
  ...
  subroutine nbody_ini ()
    ...
    character(len=360) :: redshift_file ! name of file with list of redshifts
#ifdef MH
    character(len=360) :: redshift_file_coarse ! coarse redshift list file
#endif
    ...
    if (status == 0) then
       ...
       if (len > 0) then
          ...
          dir_densWH=value(1:len)//trim(adjustl(dir_densWH_path)) &
               //trim(adjustl(dir_densWH_name))
          ...
          dir_densWH=trim(adjustl(dir_densWH_path))//trim(adjustl(dir_densWH_name))
    ...
    if (rank == 0) then
       ...
       if (.not.file_input) then
          write(*,"(A,$)") "Files with redshifts(fine, coarse): "
          write(*,"(A,$)") "(minihalo-relevant redshifts, N-body redshifts)"
       endif
       read(stdinput,*) redshift_file, redshift_file_coarse
#else
...
#ifdef MH
       ! Open and read redshift file
       open(unit=60,file=redshift_file_coarse,form="formatted",status="old")
       read(unit=60,fmt=*) NumZred_coarse
       allocate(zred_array_coarse(NumZred_coarse))
       do nz=1,NumZred_coarse
          read(unit=60,fmt=*) zred_array_coarse(nz)
       enddo
       close(60)
#endif
    ...
    call MPI_BCAST(zred_array,NumZred,MPI_DOUBLE_PRECISION,0,MPI_COMM_NEW,mympierror)
#ifdef MH
    call MPI_BCAST(NumZred_coarse,1,MPI_INTEGER,0,MPI_COMM_NEW,mympierror)
    if (rank /= 0) allocate(zred_array_coarse(NumZred_coarse))
    call MPI_BCAST(zred_array_coarse,NumZred_coarse,MPI_DOUBLE_PRECISION,0,MPI_COMM_NEW,mympierror)
#endif
#endif
    ...
    case(244)
       select case (n_box/mesh(1))
       case(64)  !!very coarse test case, 125^3
          id_str="coarsened"          
       case(32)
          id_str="coarsest"
       case(16)

...
contains
  ...
  subroutine evolve3D (dt,restart)
    ...
    if (rank == 0) then
       call system_clock(wallclock1,countspersec)
    endif
    ...
    do
       ...
       niter=niter+1
       if (rank ==0) write(6,*) 'niter', niter
       
       call pass_all_sources (niter,dt)
       
       if (rank == 0) then
          call system_clock(wallclock2,countspersec)
          if (wallclock2-wallclock1 > 15*60*countspersec) &
...
!!$    use  m_ctrper, only: ctrper
    ! ctrper seems to fail at large source number. use perm instead (Kyungjin)
    use  perm
...
!!$    if ( rank == 0 ) call ctrper (SrcSeries(1:NumSrc),1.0)
    ! ctrper seems to crash at large NumSrc, so use the following (Kyungjin)
    if ( rank == 0 ) call permi(NumSrc, SrcSeries)
...
!    real(kind=dp),parameter :: sqrt3=sqrt(3.0)
    real(kind=dp),parameter :: sqrt3=1.732050808d0
!    real(kind=dp),parameter :: sqrt2=sqrt(2.0)
    real(kind=dp),parameter :: sqrt2=1.414213562d0

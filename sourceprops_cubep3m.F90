...
module sourceprops
  ...
  integer,dimension(:,:),allocatable,public :: srcpos00 !< mesh position of high-mass sources
  integer,dimension(:,:),allocatable,public :: srcpos01 !< mesh position of low-mass sources which are NOT suppressed
  ...
  real,dimension(3) :: srcpos_input !just to read in source mesh positions in case numbers are real
  real(kind=dp),private :: srcMass00,srcMass01,total_SrcMass
  real(kind=dp),dimension(:),allocatable,public :: sM00_msun !< array of mass of high mass sources in solar mass unit
  real(kind=dp),dimension(:),allocatable,public :: sM01_msun !< array of mass of low mass sources in solar mass units
  character(len=6) :: z_str !< string value of redshift
  integer,public :: NumMassiveSrc !< counter: number of massive sources
  integer,public :: NumSupprbleSrc !< counter: number of suppressible sources
  integer,public :: NumSupprsdSrc !< counter: number of suppressed sources
...
!!$    use  m_ctrper
    use perm   ! ctrper seems to crash when too many sources exist. (Kyungjin)
    ...
    if (allocated(srcpos00)) deallocate(srcpos00)
    if (allocated(srcpos01)) deallocate(srcpos01)
    if (allocated(rsrcpos)) deallocate(rsrcpos)
    if (allocated(srcMass)) deallocate(srcMass)
    if (allocated(sM00_msun)) deallocate(sM00_msun)
    if (allocated(sM01_msun)) deallocate(sM01_msun)
...
!!$            "_sources_used_wfgamma.dat"
    ...
    call MPI_BCAST(NumMassiveSrc,1,MPI_INTEGER,0,MPI_COMM_NEW,mympierror)
    call MPI_BCAST(NumSupprbleSrc,1,MPI_INTEGER,0,MPI_COMM_NEW,mympierror)
    call MPI_BCAST(NumSupprsdSrc,1,MPI_INTEGER,0,MPI_COMM_NEW,mympierror)
#endif
             
             
#ifdef MPILOG
    if (rank /=0) write(logf,*) "Number of sources, with suppression: ",NumSrc
    if (rank /=0) write(logf,*) "Number of high-mass sources: ",NumMassiveSrc
    if (rank /=0) write(logf,*) "Number of small-mass sources, total: ",NumSupprbleSrc
    if (rank /=0) write(logf,*) "Number of small-mass sources, used: ",NumSupprbleSrc-NumSupprsdSrc
#endif
    ...
    if (NumSrc > 0) then
       ...
       allocate(srcpos00(3,NumMassiveSrc))
       allocate(srcpos01(3,NumSupprbleSrc-NumSupprsdSrc))
       allocate(sM00_msun (NumMassiveSrc))
       allocate(sM01_msun (NumSupprbleSrc-NumSupprsdSrc))
       ...
       call MPI_BCAST(srcpos00, 3* NumMassiveSrc, &
            MPI_INTEGER,0,MPI_COMM_NEW,mympierror)
       call MPI_BCAST(srcpos01, 3*(NumSupprbleSrc-NumSupprsdSrc), &
            MPI_INTEGER,0,MPI_COMM_NEW,mympierror)
       call MPI_BCAST(sM00_msun,   NumMassiveSrc, &
            MPI_DOUBLE_PRECISION,0,MPI_COMM_NEW,mympierror)
       call MPI_BCAST(sM01_msun,  (NumSupprbleSrc-NumSupprsdSrc), &
            MPI_DOUBLE_PRECISION,0,MPI_COMM_NEW,mympierror)
#endif
...
!!$          call ctrper(SrcSeries(1:NumSrc),1.0)
  ...
  subroutine establish_number_of_active_sources (restart)
    ...
    integer :: ios  ! input status for source file
    print*,"inside establish_number_of_active_sources"
    print*,"restart", restart
    if (restart == 0 .or. restart == 1) then
    sourcelistfile = trim(adjustl(sourcelistfile))
       open(unit=50,file=sourcelistfile,status='old',iostat=ios)
       print*,"ios=", ios
       if (ios .ne. 0) then !When there is no source file
          NumSrc0 = 0
          ! Report
          write(logf,*) "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
          write(logf,*) "NO SOURCE FILE ", trim(adjustl(sourcelistfile)), " Stopping!!!"
          write(6,*)    "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
          write(6,*)    "NO SOURCE FILE ", trim(adjustl(sourcelistfile)), " Stopping!!!"
          stop
       else
          ! Number of sources
          read(50,*) NumSrc0
          print*,"NumSrc0", NumSrc0
          ! Report
...
555    continue
       ...
       NumSupprsdSrc = 0
       do ns0=1,NumSrc0
          read(50,*) srcpos_input(1),srcpos_input(2),srcpos_input(3),SrcMass00,SrcMass01
          srcpos0=int(srcpos_input)
  ...
  subroutine read_in_sources (restart)
    ...
    integer :: ns
    integer :: ns0, ns00, ns01

    integer :: ios  ! input status for source file

    if (restart == 0 .or. restart == 1) then
       
       open(unit=50,file=trim(adjustl(sourcelistfile)),status='old',iostat=ios)
       ! Number of sources
       if (ios .ne. 0) then !When there is no source file
          write(logf,*) "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
          write(logf,*) "NO SOURCE FILE", sourcelistfile, " Stopping!!!"
          write(6,*)    "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
          write(6,*)    "NO SOURCE FILE", sourcelistfile, " Stopping!!!"
          stop
       else
          read(50,*) NumSrc0
       endif
       ...
       ns01 = 0
       do ns0=1,NumSrc0
          read(50,*) srcpos_input(1),srcpos_input(2),srcpos_input(3), &
               SrcMass00,SrcMass01
          srcpos0=int(srcpos_input)
          if (xh(srcpos0(1),srcpos0(2),srcpos0(3),1) < StillNeutral) then
             if (UV_Model == "Iliev et al" .or. SrcMass00 > 0.0d0) then
                ...
                if (SrcMass00 > 0d0) then
                   ns00             = ns00 + 1
                   srcpos00(1,ns00) = srcpos0(1)
                   srcpos00(2,ns00) = srcpos0(2)
                   srcpos00(3,ns00) = srcpos0(3)
                   sM00_msun (ns00) = SrcMass00*(M_grid/M_SOLAR)
                endif
                if (UV_Model == "Iliev et al") then
                   SrcMass(ns,2)=SrcMass01
                   if (srcMass01 > 0d0) then
                      ns01             = ns01 + 1
                      srcpos01(1,ns01) = srcpos0(1)
                      srcpos01(2,ns01) = srcpos0(2)
                      srcpos01(3,ns01) = srcpos0(3)
                      sM01_msun (ns01) = SrcMass01*(M_grid/M_SOLAR)
                   endif
                else
                   SrcMass(ns,2)=0.0
                endif
       ...
       do ns0=1,NumSrc
          read(50,*) srcpos_input(1),srcpos_input(2),srcpos_input(3), &
               SrcMass00,SrcMass01
!          print(*,*) 'check source input',srcpos_input,srcpos0(:,ns0)
          srcpos(1,ns0)=int(srcpos_input(1))
          srcpos(2,ns0)=int(srcpos_input(2))
          srcpos(3,ns0)=int(srcpos_input(3))

...
!       xfrac_file= "./xfrac3d_"//trim(adjustl(zred_str))//".bin"
       ...
       if (m1 /= mesh(1).or.m2 /= mesh(2).or.m3 /= mesh(3)) then
          ...
          xh_compr(:,:,:)=ionized_to_compr(xh1_real(:,:,:))
       endif
...
#ifdef MPI       
    ! Distribute the input parameters to the other nodes
    call MPI_BCAST(xh_compr,mesh(1)*mesh(2)*mesh(3),MPI_DOUBLE_PRECISION,0,&
         MPI_COMM_NEW,mympierror)
#endif
  ...
  !> Extract neutral fraction from compressed value
  elemental real(kind=dp) function neutral_from_compr(a)

    real(kind=dp),intent(in) :: a
    neutral_from_compr=(sign(0.5_dp,a)+0.5_dp)-a

  end function neutral_from_compr

  ! =======================================================================

  !> Extract ionized fraction from compressed value
  elemental real(kind=dp) function ionized_from_compr(a)

    real(kind=dp),intent(in) :: a
    
    ionized_from_compr=(0.5_dp-sign(0.5_dp,a))+a

  end function ionized_from_compr

  ! =======================================================================

  !> Find compressed value from neutral fraction
  elemental real(kind=dp) function neutral_to_compr(a)

    real(kind=dp),intent(in) :: a
    neutral_to_compr=(0.5_dp+sign(0.5_dp,a-0.5_dp))-a

  end function neutral_to_compr

  ! =======================================================================

  !> Find compressed value from ionized fraction
  elemental real(kind=dp) function ionized_to_compr(a)

    real(kind=dp),intent(in) :: a
    ionized_to_compr=(sign(0.5_dp,a-0.5_dp)-0.5_dp)+a

  end function ionized_to_compr

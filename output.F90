...
module output_module
  ...
  integer,parameter :: max_input_streams=7 !< maximum number of input streams
...
contains
  ...
  subroutine setup_output ()
    ...
    if (rank == 0) then
       if (.not.file_input) then
          ...
          write(*,*) "Enter a mask for streams 1 through 7"
          write(*,*) "E.g. 1,0,1,0,0,0,0 means streams 1 and 3, but not 2, 4, 5, 6, 7"
       endif
       read(stdinput,*) streams(1),streams(2),streams(3),streams(4),streams(5),streams(6),streams(7)
  ...
  subroutine output(zred_now,time,dt,photcons_flag)
...
#ifdef MH
    use getjLW, only: jLW
    use source_sub, only: NumAGrid, AGflag
#endif
    ...
    character(len=40) :: file1,file2,file3,file4,file5,file6,file7,file8,file9
    ...
    if (rank == 0) then
       ...
       if (streams(1).eq.1) then
          ...
          open(unit=51,file=file1,form="formatted",status="unknown") !changed to formatted by David Attard Nov 2025
...
!       endif
       ...
       ! Stream 6
#ifdef MH
       if (streams(6).eq.1) then
          write(file7,"(f6.3)") zred_now
          file7=trim(adjustl(results_dir))//"jLW3d_"//trim(adjustl(file7))//".bin"
!!$          open(unit=52,file=file7,form="binary",status="unknown")
          open(unit=52,file=file7,form="unformatted",access=binaryaccess,status="unknown")
          write(52) mesh(1),mesh(2),mesh(3)
          write(52) jLW
          close(52)
       endif
       ...
       write(zred_str,"(f6.3)") zred_now
       file8=trim(adjustl(results_dir))//"jLW_"//trim(adjustl(zred_str))//".vtk"
       open(unit=60,file=file8,status="unknown")
222    format(A)
...
224    format(A,I10)
       write(60,222)  ""
       write(60,222) "SCALARS jLW float"
       write(60,222) "LOOKUP_TABLE default"
       
       do j=   1,mesh(2)
          do i=1,mesh(1)
             write(60,*) real(jLW(i,j,mesh(3)/2))
          enddo
       enddo
       close(60)
!!$
!!$       file8=trim(adjustl(results_dir))//"AGflag_"//trim(adjustl(zred_str))//".vtk"
!!$       open(unit=60,file=file8,status="unknown")
!!$222    format(A)
!!$       write(60,222)  "# vtk DataFile Version 2.0"
!!$       write(60,222)  "Volume example"
!!$       write(60,222)  "ASCII"
!!$       write(60,222)  "DATASET STRUCTURED_POINTS"
!!$       write(60,223)  "DIMENSIONS ", mesh(1), mesh(2), mesh(3)
!!$223    format(A,3I5)
!!$       write(60,222)  "ORIGIN 0. 0. 0."
!!$       write(60,222)  "SPACING 1. 1. 1."
!!$       write(60,224)  "POINT_DATA ", mesh(1)*mesh(2)*mesh(3)
!!$224    format(A,I10)
!!$       write(60,222)  ""
!!$       write(60,222) "SCALARS AGflag integer"
!!$       write(60,222) "LOOKUP_TABLE default"
       
!!$       do j=   1,mesh(2)
!!$          do i=1,mesh(1)
!!$!!$             write(60,*) AGflag(i,j,mesh(3)/2)
!!$             write(60,*) sum(AGflag(i,j,:))
!!$          enddo
!!$       enddo
!!$       do k=      1,mesh(3)
!!$          do j=   1,mesh(2)
!!$             do i=1,mesh(1)
!!$                write(60,333) AGflag(i,j,k)
!!$333             format(I1)
!!$             enddo
!!$          enddo
!!$       enddo

...
module c2ray_parameters
  ...
  real(kind=dp),parameter :: convergence_frac=1.0e-5
...
!  integer,parameter :: max_subbox=20 !corresponds to 39 Mpc/h here, for 244/h Mpc box and 125^3 grid; max_subbox is not implemented in this (old) version!!
...
!  integer,parameter :: Number_Sourcetypes=2  ! without subgrid sources
...
!  real,dimension(Number_Sourcetypes),parameter :: phot_per_atom= (/ 10.0, 150.0 /)
  ...
  ! DO NOT WORRY OVERSPECIFYING SOURCETYPES, PHOT_PER_ATOM, ETC. IF "-DMH"
  ! PREPROCESSOR OPTION IS NOT USED IN MAKEFILE, EACH 3RD COMPONENT OF
  ! THESE DO NOT ENTER THE CALCULATION.
  integer,parameter :: Number_Sourcetypes=3  ! subgrid sources (3rd) included
  !> Source properties: Photon per atom for different source types (high to low mass)
!!$  real,dimension(Number_Sourcetypes),parameter :: phot_per_atom= &
!!$       (/ 10.0, 150.0, 3750.0 /)
  !-------- Now done with about 2 Myr timesteps (1/6)-------------------------
  ! So this corresponds (roughly) to 10, 150, 750*5
  ! For atomic cooling sources, use these numbers
  ! For Pop III, because lifetime is about 2 Myr, 
  ! better simulate with 2 Myrs or less timesteps.
!!$  real,dimension(Number_Sourcetypes),parameter :: phot_per_atom= &
!!$       (/ 2.0, 30.0, 750.0 /)
  ! ********** IMPORTANT ********************************
  ! ********** CHECK IF, COMPARED TO NON-MH SIMULATION WITH COARSER
  ! ********** TIMESTEPS, phot_per_atom IS THE RIGHT VALUE for same g factor
  ! ********** FOR 1ST AND 2ND TYPES. gfactor = fgamma/delt.
  ! ********** ALSO Ni SHOULD BE CHANGED CORRESPONDINGLY FOR 1ST AND 2ND
  ! ********** TYPES.
  ! ********** SEE THE NUMBER OF INTERMEDIATE STEPS FROM dens_morph.f90.
  real,dimension(Number_Sourcetypes),parameter :: phot_per_atom= &
       (/ 10./6., 150./6., 833. /)
  real(kind=dp),dimension(Number_Sourcetypes),parameter :: Ni  = &
       (/ 6000d0/6d0, 50000d0/6d0, 55000d0 /)
  real(kind=dp),dimension(Number_Sourcetypes),parameter :: fstar= &
       (/ 0.008d0, 0.015d0, 0.015d0 /)


  !> Source properties: Life time of sources (if set at compile time)
  real,parameter :: lifetime=11e6*YEAR

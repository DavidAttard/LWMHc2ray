...
module sourceprops
  ...
  real(kind=dp),dimension(:),allocatable,public :: sM01_msun !< array of mass of low mass sources in solar mass unit
  ...
  integer,public :: NumSupprsdSrc !< counter: number of fully suppressed sources
  real(kind=dp)        :: MassParSupprsdSrc !< suppressed amount of mass of partially suppressed sources; just for reporting purpose
  ...
  subroutine establish_number_of_active_sources (restart)
    ...
    if (restart == 0 .or. restart == 1) then
       ...
       if (ios .ne. 0) then !When there is no source file
          ...
          ! Report
          write(logf,*) "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
          write(logf,*) "NO SOURCE FILE. MUST BE INTERMEDIATE STEP!!!"
          write(6,*) "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
          write(6,*) "NO SOURCE FILE. MUST BE INTERMEDIATE STEP. OTHERWISE SOURCE DIRECTORY/ FILENAME MISSPELLED. CHECK IT OUT!!!!"
       else
          ! Number of sources
          read(50,*) NumSrc0
          ! Report
...
555    continue
       ...
       do ns0=1,NumSrc0
          ...
          ! et al source model)   
          ! NOTE THE CHANGE OF CRITERION!! XH(x,y,z,0) > StillNeutral instead
          ! of XH(x,y,z,1) < StillNeutral
          elseif (xh(srcpos0(1),srcpos0(2),srcpos0(3),0) > StillNeutral .and. &
          ...
          if (SrcMass01 /= 0.0) then
             if (xh(srcpos0(1),srcpos0(2),srcpos0(3),0) < StillNeutral .or. &
               UV_Model /= "Iliev et al") NumSupprsdSrc=NumSupprsdSrc+1
          endif
       enddo
       close(50)
       write(logf,*) "Number of suppressable sources: ",NumSupprbleSrc
       write(logf,*) "Number of fully suppressed sources: ",NumSupprsdSrc
  ...
  subroutine read_in_sources (restart)
    ...
    if (restart == 0 .or. restart == 1) then
       ...
       do ns0=1,NumSrc0
          ...
          ! if ionized with fraction from StillNeutral to 99%,
          ! partially suppress SrcMass01 by ionized fraction.
          if (xh(srcpos0(1),srcpos0(2),srcpos0(3),1) < 0.99_dp) then

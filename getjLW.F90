...
!
  ...
  use sizes, only: mesh
  use nbody, only: zred_array, binaryaccess
  ...
  use jLWgreen, only: greenK, read_greenK, get_rLW, rLW_zobs, HcOm
  use c2ray_parameters, only: phot_per_atom, Ni, fstar
  use cosmology_parameters, only: Omega0, Omega_B
...
!!$    open(unit=33,file=filej,form="binary",status="old")
    open(unit=33,file=filej,form="unformatted",access=binaryaccess,status="old")
    read(33) m1,m2,m3
...
!!$    open(unit=10, file=fname, form='binary', status='unknown')
    open(unit=10, file=fname, form='unformatted', access=binaryaccess, status='unknown')
...
!!$    open(unit=10, file=fname, form='binary', status='old')
    open(unit=10, file=fname, form='unformatted', access=binaryaccess, status='old')

...
!!$  integer, public, parameter :: MHflag = 0
  ...
  real   (kind=si),dimension(:), allocatable, public :: zred_array_interp
  ...
  character(len=100), parameter :: MHfit_file="zred_halodelta1_nMHMpc3_Planck" !input data for MH subgrid model
...
contains
  ...
  subroutine AGrid_properties(nz,AGlifetime,jLWgrid)
    ...
    integer :: debug_print_limit
    integer :: xh_failing = 0    
    integer :: jLW_failing = 0
    integer :: diff_subsrcMsun_failing = 0
    ...
    if (rank == 0) then
       ...
       print*,"zred_now,  densNDcrit", zred_now,  densNDcrit
             ...
             do i=1,mesh(1)
                ...
                if (nz > 1) then
                   diff_subsrcMsun = &
                        ...
                        - subsrcM_msun(zred_prev,dens_ND_prev(i,j,k),    &
                        densNDcrit_prev)
                else
                   diff_subsrcMsun= &
                        subsrcM_msun(zred_now,dens_ND(i,j,k),densNDcrit)
...
!!$                     .AND. subsrcM_msun(zred_now,dens_ND(i,j,k),densNDcrit) > 0d0) then
                ...
                if (xh(i,j,k,1) < StillNeutral &
                  ...
                  if (debug_print_limit > 0) then
                     print*, "Cell failing at (i,j,k)=", i,j,k
                     print*, "   xH = ", xh(i,j,k,1)
                     print*, "   StillNeutral = ", StillNeutral
                     print*, "   jLW   = ", jLWgrid(i,j,k)
                     print*, "   jLWcrit_now = ", jLWcrit_now
                     print*, "   diff_subsrcMsun = ", diff_subsrcMsun
                     write(6,*)
                     debug_print_limit = debug_print_limit - 1
                  end if
                  if (xh(i,j,k,1) > StillNeutral) then
                     xh_failing = xh_failing + 1
                  endif
                  if (jLWgrid(i,j,k) > jLWcrit_now) then
                     jLW_failing = jLW_failing + 1
                  endif
                  if (diff_subsrcMsun <= 0d0) then
                     diff_subsrcMsun_failing = diff_subsrcMsun_failing + 1
                  endif
       ...
       print*,"Total xh_failing = ",xh_failing 
       print*,"Total jLW_failing = ",jLW_failing 
       print*,"Total diff_subsrcMsun_failing = ",diff_subsrcMsun_failing 
       print*,"NumAGrid = ", NumAGrid
       print*,"Total = ", diff_subsrcMsun_failing + NumAGrid
       print*," Total grids = ", 125*125*125
...
!!$       open(unit=20, file=trim(adjustl(MHfit_file)), form="binary", status="old")
       ...
       read(20) Nzdata, Ndelta1data  !number of redshifts and delta bins
       !       read(20) zmin, zmax, LGdelta1min, LGdelta1max
       print*,'check Nzdata', Nzdata, Ndelta1data
       !! redshifts are now in an array and NOT assumed to be regularly spaced
       if (allocated(zred_array_interp)) deallocate(zred_array_interp)
       allocate(zred_array_interp(Nzdata))       

       read(20) LGdelta1min, LGdelta1max, zred_array_interp
       print*,'check zred', LGdelta1min, LGdelta1max, zred_array_interp
       
       if (rank==0) write(6,*)  '.................'
       if (rank==0) write(6,*)  Nzdata, Ndelta1data, zred_array_interp(1), zred_array_interp(Nzdata), LGdelta1min, LGdelta1max
...
!    call MPI_BCAST(zmin,       1,                  MPI_DOUBLE_PRECISION,0,MPI_COMM_NEW,mympierror)
    ...
    call MPI_BCAST(zred_array_interp,Nzdata,       MPI_DOUBLE_PRECISION,0,MPI_COMM_NEW,mympierror)

             ...
             do i=1,j
                ...
                if (dtauMpc_sm <= ros .and. ros <= dtauMpc_bg) then
                   ...
                   if (ros < 0.1d0*d_boxMpc) then
                      green(i,j,k) = green(i,j,k) + 3d0/(4d0*pi*d_boxMpc**2)
                   else
...
!!$    open(unit=60,file=fileout,form='binary',status="unknown")  !! not working in gfortran
    open(unit=60,file=fileout,form=densityformat,access=densityaccess,status="unknown")

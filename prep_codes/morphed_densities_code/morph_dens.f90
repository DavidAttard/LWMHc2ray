...
!  use sizes, only: mesh
  ...
  integer,dimension(3),parameter :: mesh=(/125,125,125/)
...
! when doing the density fields with haloes included 
  character(len=512),parameter :: dir_dens="../halos_included/"
  character(len=512),parameter :: dir_morphed_dens="../morphed_all/"
  character(len=512),parameter :: filename_suffix="ntotcoarse_all.dat"

! when doing the density fields with haloes excluded 
!  character(len=512),parameter :: dir_dens="../halos_removed/"
!  character(len=512),parameter :: filename="ncoarse_all.dat"
  ...
  do ii=2, N_zred
     ...
     dens_prev_file=trim(adjustl(dir_dens))//trim(adjustl(z_prev_str))//trim(adjustl(filename_suffix))
!     "ntot_all.dat"
     dens_file     =trim(adjustl(dir_dens))//trim(adjustl(z_str))//trim(adjustl(filename_suffix))
!"ntot_all.dat"
     
!     open(unit=20,file=dens_prev_file,form="binary",status="old")
     open(unit=20,file=dens_prev_file,form=densityformat,access=binaryaccess,status="old")
     read(20) m1,m2,m3
     print *, 'mesh number', m1, m2, m3
     print*,'mesh expected',mesh
     if (m1 /= mesh(1)) then
     ...
     dens_fine_file = trim(adjustl(dir_morphed_dens))//trim(adjustl(z_prev_str))//trim(adjustl(filename_suffix))
!"n_all.dat"
!     open(unit=20,file=dens_fine_file,form="binary",status="unknown")
     open(unit=20,file=dens_fine_file,form=densityformat,access=binaryaccess,status="unknown")
     write(20) m1,m2,m3
     write(20) ndens_prev
     close(20)
...
!!$     close(1)
     ...
     do i=1, N_inter
        ...
        ! density file spaced in finer time sequence: for z_prev
        dens_fine_file = trim(adjustl(z_inter_str))//trim(adjustl(filename_suffix))
!"n_all.dat"
!        open(unit=20,file=dens_fine_file,form="binary",status="unknown")
        open(unit=20,file=dens_fine_file,form=densityformat,access=binaryaccess,status="unknown")
        write(20) m1,m2,m3

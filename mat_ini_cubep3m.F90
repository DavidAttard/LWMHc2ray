module material
  ...
  use nbody, only: nbody_type, M_grid, M_particle, id_str, dir_dens, dir_densWH, NumZred, Zred_array
  use nbody, only: densityformat, densityheader, densityWHformat, binaryaccess, densityWHheader, clumpingformat, clumpingheader, density_unit
...
#ifdef MH
  real(kind=si) :: dens_ND(mesh(1),mesh(2),mesh(3)) !< nondimensional cell density. (cell density with halos)/(average density with halos).
  real(kind=si) :: dens_ND_prev(mesh(1),mesh(2),mesh(3)) !< nondimensional cell density at previous z. (cell density with halos)/(average density with halos).
#endif
...
contains
  ...
  subroutine mat_ini (restart, nz0, ierror)
    ...
    if (nbody_type /= "cubep3m") then
       ...
       if (rank == 0) then
          ...
          read(stdinput,*) answer
          if (answer == "y" .or. answer == "Y") restart=2
          if (.not.file_input) write(*,"(A,$)") "Number of starting slice(s): (1 number for no MH, 2 - fine and coarse -- for MH)"
          
#ifdef MH
          ! nz0 for finely space redshifts; nz0_coarse for coarse redshifts
          read(stdinput,*) nz0, nz0_coarse
#else
          read(stdinput,*) nz0
#endif
...
#ifdef MH
       call MPI_BCAST(nz0_coarse,1,MPI_INTEGER,0,MPI_COMM_NEW,mympierror)
#endif
#endif
  ...
  subroutine dens_ini (zred_now,nz)
    ...
    character(len=512):: dens_file, densWH_file
...
!!$            "-"//trim(adjustl(id_str))//"n_all.dat"  ! kyungjins files
!!$            "n_all.dat"  ! original
            "ncoarsened_all.dat"  ! for the coarsened case (fast tests of code)
...
!!$       open(unit=20,file=dens_file,form=densityformat,status="old")
       open(unit=20,file=dens_file,form=densityformat,access=binaryaccess,status="old")
...
#ifdef MH
       ! Get dimensionless density, INCLUDING halos.
       ! Make ndens into double precision when summing. Summing
       ! a large number of real type arrays can result in errors!!
       densWH_file=trim(adjustl(dir_densWH))// &
!            trim(adjustl(zred_str))//"n_all.dat"
            trim(adjustl(zred_str))//"ntotcoarsened_all.dat" !for fast tests of code
            write(unit=logf,fmt="(4A)") "Reading ",id_str, &
            " density (with halos) input from ",trim(densWH_file)

!!$       open(unit=20,file=densWH_file,form=densityWHformat,status="old")
       open(unit=20,file=densWH_file,form=densityWHformat,access=binaryaccess,status="old")
       ! Read in data
       if (densityWHheader) then
          read(20) m1,m2,m3
          if (m1 /= mesh(1).or.m2 /= mesh(2).or.m3 /= mesh(3)) then
             write(logf,*) "Warning: file with densities unusable"
             write(logf,*) "mesh found in file: ",m1,m2,m3
       ...
       dens_ND = real(ndens_real) / &
            (real(sum(dble(ndens_real)))/real(mesh(1)*mesh(2)*mesh(3)))
...
#ifdef MH
    call MPI_BCAST(dens_ND,mesh(1)*mesh(2)*mesh(3),MPI_REAL          ,0,&
         MPI_COMM_NEW,mympierror)
#endif
#endif
    ...
    avg_dens=sum(ndens)/real(mesh(1)*mesh(2)*mesh(3),dp)
    ...
    if (rank == 0) then
       ...
       write(logf,"(A,1pe10.3,A)") &
            "Average density (summed in double precision) = ", &
            sum(real(ndens,dp))/real(mesh(1)*mesh(2)*mesh(3),dp)," cm^-3"
  ...
  subroutine clumping_init (zred_now)
    ...
    if (rank == 0) then
       ...
       clump_file=trim(adjustl(dir_dens))// &
            trim(adjustl(zred_str))// &
!!$            "-"//trim(adjustl(id_str))//"c_all.dat"   !kyungjins files
            "c_all.dat"     !original
...
!!$       open(unit=20,file=clump_file,form=clumpingformat,status="old")
       open(unit=20,file=clump_file,form=clumpingformat,access=binaryaccess,status="old")
    ...
    clumping=sum(real(clumping_grid,dp))/real(mesh(1)*mesh(2)*mesh(3),dp)
    if (rank == 0) then
